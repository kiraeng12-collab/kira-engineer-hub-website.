(function () {
  ...
  function render(showManage) {
    ...
    banner.style.cssText = "position:fixed;left:16px;right:16px;bottom:16px;z-index:var(--z-cookie-banner);max-width:760px;margin:auto;background:var(--surface);color:var(--ink);border:1px solid var(--border);border-radius:var(--radius);padding:18px;box-shadow:var(--shadow);font-family:inherit";
    banner.innerHTML = '<h2 style="font-size:1.1rem;margin:0 0 8px">' + t.title + '</h2><p style="margin:0 0 12px;color:var(--muted)">' + t.body + '</p>' +
      ...
      '<p style="font-size:.82rem;color:var(--muted);margin:0">' + t.necessary + ' cookies are always active. Optional categories are not preselected.</p></div>' : "") +
      '<div style="display:flex;flex-wrap:wrap;gap:8px"><button class="button" data-cookie-accept>' + t.accept + '</button><button class="button secondary" data-cookie-reject>' + t.reject + '</button><button class="button secondary" data-cookie-manage>' + (showManage ? t.save : t.manage) + '</button></div>';
